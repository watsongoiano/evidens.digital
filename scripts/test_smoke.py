...
def post_json(client, path: str, payload: dict, headers: dict | None = None):
    return client.post(path, data=json.dumps(payload), content_type='application/json', headers=headers or {})
...
def main():
    ...
    assert_true(any((rec.get('referencia_html') or '') for rec in recs1), "No referencia_html found in recommendations (25M)")
    ...
    assert_true(any('vpc15' in t or 'vpc13' in t for t in titles2), "Pneumococcal VPC15/VPC13 not found for 55M")
    assert_true(any('vpp23' in t for t in titles2), "Pneumococcal VPP23 not found for 55M")
